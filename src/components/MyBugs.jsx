...
function MyBugs({ currentUser }) {
  ...
  const loadMyBugs = async (showLoading = false) => {
    try {
      if (showLoading) {
        setLoading(true);
      } else {
      ...
      // First refresh all bugs from backend to ensure we have latest data
      await refreshBugs();
      // Then filter for current user's bugs
      const myBugs = await getMyAssignedBugs();
      setBugs(myBugs);
      console.log('Loaded my bugs:', myBugs.length);
    } catch (error) {
      console.error('Error loading my bugs:', error);
    } finally {
      if (showLoading) {
        setLoading(false);
      } else {
